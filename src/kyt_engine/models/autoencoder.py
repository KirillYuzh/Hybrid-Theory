...
class AutoencoderDetector(_BaseDetector):
    def __init__(
        self,
        latent_dim: int = 32,
        epochs: int = 100,
...
class _VAE(nn.Module):
    def __init__(self, input_dim: int, latent_dim: int = 16) -> None:
        super().__init__()
        enc_dim = max(latent_dim * 4, input_dim // 2)
        self._encoder = nn.Sequential(
            nn.Linear(input_dim, enc_dim),
            nn.ReLU(),
        ...
        self._mu = nn.Linear(enc_dim // 2, latent_dim)
        self._logvar = nn.Linear(enc_dim // 2, latent_dim)
        self._decoder = nn.Sequential(
            nn.Linear(latent_dim, enc_dim // 2),
            nn.ReLU(),
...
class VAEDetector(_BaseDetector):
    def __init__(
        self,
        latent_dim: int = 16,
        epochs: int = 30,
        ...
        self._net: _VAE | None = None

    def fit(self, X: pd.DataFrame, y: pd.Series | np.ndarray) -> VAEDetector:
        torch.manual_seed(self._random_state)
        np.random.seed(self._random_state)
        ...
        n_features = X_scaled.shape[1]
        self._net = _VAE(n_features, self._latent_dim).to(self._device)
        optimizer = torch.optim.Adam(self._net.parameters(), lr=self._lr)
